...
.PARAMETER Path
    Test files or folders. Default: tests\core, tests\pinball, tests\lightgun and tests\gui.
.PARAMETER Local
...
if (-not $Path) {
    $Path = if ($Local) { @(Join-Path $testsDir 'local') } else { @((Join-Path $testsDir 'core'), (Join-Path $testsDir 'pinball'), (Join-Path $testsDir 'lightgun'), (Join-Path $testsDir 'gui')) }
}
