...
if ($WhatIfPreference -and $sourceRoot -and $targetRoot -and -not $locked) {
...
$step = New-KitStep -Name 'pinball-4-copy' `
    -Test { [bool]($sourceRoot -and $targetRoot) -and -not $locked } `
    -Invoke {
    ...
    -Verify {
        if (-not ($sourceRoot -and $targetRoot)) { return $false }
